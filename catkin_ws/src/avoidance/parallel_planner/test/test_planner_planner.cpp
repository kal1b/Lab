...
#include "../include/parallel_planner/parallel_planner.h"
...
class PlannerPlannerTests : public ::testing::Test {
 public:
  PlannerPlanner planner;

  void SetUp() override {
    ros::Time::init();

    planner.setDefaultPx4Parameters();
    avoidance::PlannerPlannerNodeConfig config = avoidance::PlannerPlannerNodeConfig::__getDefault__();
...
TEST_F(PlannerPlannerTests, no_obstacles) {
  // GIVEN: a Planner planner, a scan with no obstacles, pose and goal
  planner.original_cloud_vector_.emplace_back();

  // WHEN: we run the Planner planner
  planner.runPlanner();
  planner.runPlanner();
...
TEST_F(PlannerPlannerTests, all_obstacles) {
  // GIVEN: a Planner planner, a scan with obstacles everywhere, pose and goal
  float shift = 0.f;
  float distance = 2.f;
  ...
  // WHEN: we run the Planner planner again
  planner.runPlanner();
...
TEST_F(PlannerPlannerTests, obstacles_right) {
  // GIVEN: a Planner planner, a scan with obstacles on the right, pose and goal
  float shift = -0.5f;
  float distance = 2.f;
  float fov_half_y = distance * std::tan(planner.getHFOV(0) * TO_RAD / 2.f);
  float max_y = shift + fov_half_y, min_y = shift - fov_half_y;
...
TEST_F(PlannerPlannerTests, obstacles_left) {
  // GIVEN: a Planner planner, a scan with obstacles on the left, pose and goal
  float shift = 0.5f;
  float distance = 2.f;
  float fov_half_y = distance * std::tan(planner.getHFOV(0) * TO_RAD / 2.f);
  float max_y = shift + fov_half_y, min_y = shift - fov_half_y;
