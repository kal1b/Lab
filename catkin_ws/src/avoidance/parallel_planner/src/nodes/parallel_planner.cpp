#include "parallel_planner/parallel_planner.h"

#include "avoidance/common.h"
#include "parallel_planner/star_planner.h"
...
ParallelPlanner::ParallelPlanner() : star_planner_(new StarPlanner()) {}

ParallelPlanner::~ParallelPlanner() {}

// update UAV pose
void ParallelPlanner::setState(const Eigen::Vector3f& pos, const Eigen::Vector3f& vel, const Eigen::Quaternionf& q) {
  position_ = pos;
  velocity_ = vel;
...
void ParallelPlanner::dynamicReconfigureSetParams(avoidance::ParallelPlannerNodeConfig& config, uint32_t level) {
...
void ParallelPlanner::setGoal(const Eigen::Vector3f& goal) {
  goal_ = goal;

  ROS_INFO("===== Set Goal ======: [%f, %f, %f].", goal_.x(), goal_.y(), goal_.z());
  applyGoal();
}
void ParallelPlanner::setPreviousGoal(const Eigen::Vector3f& prev_goal) { prev_goal_ = prev_goal; }

void ParallelPlanner::setFOV(int i, const FOV& fov) {
...
Eigen::Vector3f ParallelPlanner::getGoal() const { return goal_; }

void ParallelPlanner::applyGoal() { star_planner_->setGoal(goal_); }

void ParallelPlanner::runPlanner() {
...
void ParallelPlanner::create2DObstacleRepresentation(const bool send_to_fcu) {
...
void ParallelPlanner::generateHistogramImage(Histogram& histogram) {
...
void ParallelPlanner::determineStrategy() {
  // clear cost image
...
void ParallelPlanner::updateObstacleDistanceMsg(Histogram hist) {
  sensor_msgs::LaserScan msg = {};
  msg.header.stamp = ros::Time::now();
  msg.header.frame_id = "parallel_origin";
  ...
  for (int i = 0; i < GRID_LENGTH_Z; ++i) {
    // turn idxs 180 degress to point to Parallel north instead of south
...
void ParallelPlanner::updateObstacleDistanceMsg() {
  sensor_msgs::LaserScan msg = {};
  msg.header.stamp = ros::Time::now();
  msg.header.frame_id = "parallel_origin";
  msg.angle_increment = static_cast<double>(ALPHA_RES) * M_PI / 180.0;
...
Eigen::Vector3f ParallelPlanner::getPosition() const { return position_; }

const pcl::PointCloud<pcl::PointXYZI>& ParallelPlanner::getPointcloud() const { return final_cloud_; }

void ParallelPlanner::setDefaultPx4Parameters() {
...
void ParallelPlanner::getTree(std::vector<TreeNode>& tree, std::vector<int>& closed_set,
...
void ParallelPlanner::getObstacleDistanceData(sensor_msgs::LaserScan& obstacle_distance) {
  obstacle_distance = distance_data_;
}

avoidanceOutput ParallelPlanner::getAvoidanceOutput() const {
  avoidanceOutput out;
