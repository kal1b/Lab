#include "parallel_planner/parallel_planner_visualization.h"

#include "avoidance/common.h"
#include "parallel_planner/planner_functions.h"
#include "parallel_planner/tree_node.h"
...
// initialize subscribers for Parallel planner visualization topics
void ParallelPlannerVisualization::initializePublishers(ros::NodeHandle& nh) {
...
void ParallelPlannerVisualization::visualizePlannerData(const ParallelPlannerV & planner,
...
void ParallelPlannerVisualization::publishFOV(const std::vector<FOV>& fov_vec, float max_range) const {
...
void ParallelPlannerVisualization::publishRangeScan(const sensor_msgs::LaserScan& scan,
                                                 const Eigen::Vector3f& newest_position) const {
  visualization_msgs::Marker m;
  m.header.frame_id = "parallel_origin";
  m.header.stamp = ros::Time::now();
  m.id = 0;
  m.type = visualization_msgs::Marker::TRIANGLE_LIST;
  m.action = visualization_msgs::Marker::ADD;
...
void ParallelPlannerVisualization::publishOfftrackPoints(Eigen::Vector3f& closest_pt, Eigen::Vector3f& deg60_pt) {
  visualization_msgs::Marker m;
...
void ParallelPlannerVisualization::publishTree(const std::vector<TreeNode>& tree, const std::vector<int>& closed_set,
                                            const std::vector<Eigen::Vector3f>& path_node_positions) const {
  visualization_msgs::Marker tree_marker;
  tree_marker.header.frame_id = "parallel_origin";
  ...
  path_marker.header.frame_id = "parallel_origin";
...
void ParallelPlannerVisualization::publishGoal(const geometry_msgs::Point& goal) const {
...
void ParallelPlannerVisualization::publishDataImages(const std::vector<uint8_t>& histogram_image_data,
...
void ParallelPlannerVisualization::visualizeWaypoints(const Eigen::Vector3f& goto_position,
  ...
  sphere1.header.frame_id = "parallel_origin";
  ...
  sphere2.header.frame_id = "parallel_origin";
  ...
  sphere3.header.frame_id = "parallel_origin";
...
void ParallelPlannerVisualization::publishPaths(const Eigen::Vector3f& last_position,
  ...
  path_actual_marker.header.frame_id = "parallel_origin";
  ...
  path_waypoint_marker.header.frame_id = "parallel_origin";
  ...
  path_adapted_waypoint_marker.header.frame_id = "parallel_origin";
...
void ParallelPlannerVisualization::publishCurrentSetpoint(const geometry_msgs::Twist& wp,
                                                       const PlannerState& waypoint_type,
                                                       const Eigen::Vector3f& newest_position) const {
  visualization_msgs::Marker setpoint;
  setpoint.header.frame_id = "parallel_origin";
